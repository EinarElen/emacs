...
- Location of some extra snippets for the yasnippet package
  ...
  later in [[*Yasnippet][Yasnippet]]. We also have our own in [[elisp:(find-variable 'elib-snippets-directory)][`elib-snippets-directory']]
...
(add-to-list-multi
 ...
 none\n?\n#+END_SRC")
   ...
   ("dfn" "\\begin{defn}{?}\n\n\\end{defn}")
   ("ex" "\\begin{exmp}?\n\n\\end{exmp}")
   ("ii" "#+BEGIN_SRC ipython :session :exports both :results output raw drawer\n%matplotlib inline\n#+END_SRC")
   ("ip" "#+BEGIN_SRC ipython :session :exports both :results output raw drawer\n#+END_SRC")
   ("lh"
...
(defun elib-exwm-config ()
  ...
  (exwm-input-set-key (kbd "s-SPC") #'elib-toggle-kbd-languaged)
