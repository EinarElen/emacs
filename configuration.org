...
#+BEGIN_SRC bash
sudo dnf install llvm* clang* cmake 
git clone https://github.com/MaskRay/ccls.git ~/src/ccls --depth=1 --recursive
mkdir -pv ~/src/build/ccls
cd ~/src/build/ccls
cmake -GNinja -BRelease -H../../ccls -DCMAKE_INSTALL_PREFIX=~/.local 
cd Release
ninja
ninja install
cd ~/src
